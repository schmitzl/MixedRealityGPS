...
var graffitiTramSceneGeoPos = Cartesian3.fromDegrees(18.062136, 59.328058, 22.89); //(17.920747, 59.374212, 11.97);
...
loadSchedule();
var scheduleGeoPos = Cartesian3.fromDegrees(18.062136, 59.328058, 22.89); //(17.920747, 59.374212, 11.97);
...
app.updateEvent.addEventListener(function (frame) {
    ...
    if (step == graffiti_step) {
        ...
        if (!isObjInit) {
            scene.add(graffitiTramScene);
            isObjInit = true;
            userStartPose = app.context.getEntityPose(app.context.user);
        }

        var graffitiScenePos = app.context.getEntityPose(graffitiTramSceneGeoEntity);
        if (isUsingUserPose) {
            graffitiTramScene.position.copy(userStartPose.position);
        } else {
        ...
        if (isUsingUserPose) {
            tramScene.position.copy(userStartPose.position);
        } else {
        ...
        if (isSearching) {
            ...
            tramScene.rotation.y = Math.atan2((camera.position.x - tramScene.position.x), (camera.position.z - tramScene.position.z)) + Math.PI;
        ...
        if (isUsingUserPose) {
            schedule.position.copy(userStartPose.position);
        } else {
